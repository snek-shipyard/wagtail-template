...
class Page(interface_cls):
    ...
    def resolve_url_path(self, info: ResolveInfo) -> str:
        ...
        url = (
            self.url_path
            if not self.url_path.startswith(url_prefix)
            else self.url_path[len(url_prefix) :]
        )
        return url.rstrip("/")
    ...
    def resolve_children(self, info: ResolveInfo, **_kwargs):
        ...
        return (
            with_page_permissions(info.context, query.specific())
            .live()
            .order_by("path")
            .all()
        )
...
def PagesQueryMixin():  # noqa: C901
    class Mixin:
        if RELAY:
            ...
            pages = graphene.List(
                Page, token=graphene.String(required=False), parent=graphene.Int()
            )
        ...
        preview = graphene.Field(
            Page, token=graphene.String(required=False), id=graphene.Int(required=True),
        )

        preview_add = graphene.Field(
            Page,
            token=graphene.String(required=False),
        ...
        def resolve_page(
            ...
            **_kwargs,
            ...
            page = with_page_permissions(info.context, query.specific()).live().first()
        ...
        @login_required
        def resolve_preview_add(
            self,
            info: ResolveInfo,
            app_name: str = "wagtailcore",
            model_name: str = "page",
            parent: int = None,
            **_kwargs,
        ):  # pragma: no cover
        ...
        # Show in Menu
        show_in_menus = graphene.List(Page, token=graphene.String(required=False),)
        ...
        def resolve_show_in_menus(self, info: ResolveInfo, **_kwargs):
            ...
            return (
                with_page_permissions(
                    info.context, wagtailPage.objects.filter(show_in_menus=True)
                )
                .live()
                .order_by("path")
            )
